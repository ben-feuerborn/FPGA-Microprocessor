...
//////////////////////////////////////////////////////////////////////////////////
    ...
    wire mat_control;
    wire mab_control;
    wire reg_dst_control;
    wire reg_write_control;
    wire mem_write_control;
    wire mem_read_control;
    wire [1:0] whb_control;
    wire mem_to_reg_control;
    wire branch_control;
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    Mux32Bit2To1 mux_new_old_pc(
        .select(pc_write),// 1-bit input
        .in_A(pc_next_if_id - 4),// 32-bit input
        .in_B(pc_src),// 32-bit input
        .out(pc_src_real)// 32-bit output
    );
    
    ProgramCounter pc(
        .Clk(Clk),                                  // 1-bit input
        .Reset(Rst),                                // 1-bit input
        .Address(pc_src_real),                           // 32-bit input
...
//////////////////////////////////////////////////////////////////////////////////
    Register_IF_ID register_if_id(
        .Clk(Clk),                                  // 1-bit input
        .if_id_write(if_id_write),// 1-bit input
...
//////////////////////////////////////////////////////////////////////////////////
    ...
    HazardDetectionUnit hazard_detection_unit(
        .rs(instruction_if_id[25:21]),              // 5-bit input
        .rt(instruction_if_id[20:16]),              // 5-bit input
        .RegDest_ex_mem(reg_dest_ex_mem),           // 5-bit input
        .RegDest_mem_wb(reg_dest_mem_wb),           // 5-bit input
        .RegWrite_ex_mem(reg_write_ex_mem),         // 1-bit input
        .RegWrite_mem_wb(reg_write_mem_wb),         // 1-bit input
        .pc_write(pc_write),                        // 1-bit output
        .if_id_write(if_id_write),                  // 1-bit output
        .stall(stall)                               // 1-bit output
    );
    
    ControlMux control_mux(
        .stall(stall),                              // 1-bit input
        ...
        .AluOp_out(alu_op_control),                 // 4-bit output
        .MAT_out(mat_control),                      // 1-bit output
        .MAB_out(mab_control),                      // 1-bit output
        .RegDst_out(reg_dst_control),               // 1-bit output
        .RegWrite_out(reg_write_control),           // 1-bit output
        .MemWrite_out(mem_write_control),           // 1-bit output
        .MemRead_out(mem_read_control),             // 1-bit output
        .WHB_out(whb_control),                      // 2-bit output
        .MemtoReg_out(mem_to_reg_control),          // 1-bit output
        .Branch_out(branch_control)                 // 1-bit output
    );
...
//////////////////////////////////////////////////////////////////////////////////
    Register_ID_EX register_id_ex(
        ...
        .AluOp_in(alu_op_control),                          // 4-bit input
        .MAT_in(mat_control),                               // 1-bit input
        .MAB_in(mab_control),                               // 1-bit input
        .RegDst_in(reg_dst_control),                        // 1-bit input
        .RegWrite_in(reg_write_control),                    // 1-bit input
        .MemWrite_in(mem_write_control),                    // 1-bit input
        .MemRead_in(mem_read_control),                      // 1-bit input
        .WHB_in(whb_control),                               // 2-bit input
        .MemtoReg_in(mem_to_reg_control),                   // 1-bit input
        .Branch_in(branch_control),                         // 1-bit input
